...
def selection_sort(ls): 
    # theta: N^2
    for current in range(len(ls)): #N
        min_index = find_index_of_min(ls, current, len(ls)) #N  
...
def insertion_sort(ls):
    ...
    # relatively simple to implement, inefficient for large datasets
...
def merge_sort(ls):
    def merge(ls, left_ind, left_end, right_end, temp): 
        # constant time complexity (omega,theta,bigO): NlogN
        # stable, suitable for sorting large datasets ^^, 
        # time complexity same regarless of dataset size
        # but temp list takes up memory space
...
##    print(ls);
    ...
    for j in range(10):
        ...
        quick_sort(ls)
        elapsed=t.elapsed()
        print("Sorting",len(ls),"elements took",elapsed,"seconds h0")
